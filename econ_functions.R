...
# the impacts of covid on gender
#################################################################################################
  ...
  for (v in outcomes){
    ...
    for (c in covs){
      ...
      if (source == "All") {
        ...
        if (weighted == T) {
          ip <- dat[dat$source == source, c(v, s, "location_name", "weight")]
        } else {
          ip <- dat[dat$source == source, c(v, s, "location_name")]
        }
        ip <- ip[complete.cases(ip) & !is.infinite(ip[[v]]), ]
      }
      # Common for both
      ...
      if (skip == 1) {
        ...
        cof <- data.table(variable=row.names(summary_mod$coefficients),summary_mod$coefficients,outcome=v,N=nobs(mod),
                          N_male=dim(ip[ip$female==0,])[1],
                          N_female=dim(ip[ip$female==1,])[1],
