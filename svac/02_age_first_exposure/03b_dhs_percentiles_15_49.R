...
# 
# Author: USERNAME
# Purpose: DHS distributions of age at first SV experience - sensitivity analysis including all ages 15-49
#
####################################################################################################################################################################
...
#examine correlation between participant age and reported age at first exp
cor(x=data2$age_year, y=data2$age_first_exp, method='pearson', use='complete.obs')

# apply <50% missingness exclusion criteria
data2 <- data2[age_first_exp_missingness<=0.5]

#group by super-region
setorder(data2, super_region_name)
data2[, survey_year:=factor(survey_year, unique(data2$survey_year))]

#number plug for results text
np <- copy(data2)
