...
# Purpose: Linear model for underreporting adjustment
#
####################################################################################################################################################################
rm(list=ls())

#date 
ur_xw_date <- "MODEL-DATE"

#version name
version <- paste0("VERSION-NAME")
...
#subset to actual NIDs (ie, not custom aggregates)
data <- copy(og_data[!is.na(nid)])

#drop extra columns
data[, c("cases", "lancet_label", "lower", "upper", "age_range", "csa_age", "case_definition", "title") := NULL]

#subset based on what we want
xwalk_data <- data[(collection_method %in% c("face", "card")) | (ihme_loc_id == 'BHU' & collection_method == 'both')] #only have both from south asia
xwalk_data[collection_method == 'both', collection_method := 'card']
...
#linear regression to get the slope and intercept
