...
stage_version <- paste0("STAGE-NAME")


##### 0. SET UP ##########################################################################################################################

#libraries + central functions
pacman::p_load(data.table, dplyr, openxlsx, ggplot2, stringr, lme4)
...
#get data
mdata <- fread(paste0("FILEPATH", 'male_csa_xw-', m_xwid, '_predicted_inputs.csv'))
fdata <- fread(paste0("FILEPATH", 'fem_csa_xw-', f_xwid, '_predicted_inputs.csv'))

#bind male and female data together
data <- rbind(mdata, fdata, fill=T)
orig <- copy(data)

#subset to only columns needed for the regression
data <- data[is_outlier == 0, c('nid', 'location_id', 'ihme_loc_id', 'sex', 'year_id', 'age_group_id', 'val', 'variance')]
...
saveRDS(model, paste0("FILEPATH", 'custom_stage1_lmer_fem', f_xwid, '_male', m_xwid, '.RDS'))


##### 3. PREDICT FROM MODEL ##############################################################################################################

#create prediction data frame for every location-year-age-sex combinations
#note: we usually model one year into the future (eg, SDG 2022 is modeled through 2023 and later subset)
